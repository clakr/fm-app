...
export default async function createAuthorModalStyle() {
  const content = `@import url("https://fonts.googleapis.com/css2?family=Source+Code+Pro:wght@700&family=Wix+Madefor+Display:wght@400;600;700&display=swap");
...
.modal__list > dt > a:is(:active, :hover) {
...
.modal__icons > a > svg:is(:hover, :active) {
...
`;
  ...
  writeFile("src/_authorModal.css", content, (err) => {
    if (err) throw err;
  });
}
